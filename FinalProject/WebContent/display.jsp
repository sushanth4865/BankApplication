...
<body>
	<%@include file="header.jsp" %>
		...
		%>
			From<input type="text" name="from" value=<%out.print("'"+from+"'"); %>>
			To<input type="text" name="to" value=<%out.print("'"+to+"'"); %>>
...
<% 
		...
		ResultSet rs=st.executeQuery("SELECT `number`, `date`, `description`, `chagenum`, `withdrawamt`, `depositmt`, `availablebal`, `userid` FROM `transactions` WHERE userid='"+signedIn+"' AND date >= '"+from+"' AND date <= '"+to+"'");
